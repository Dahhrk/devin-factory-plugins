...
- **Paint budget.** HUDPaint and Derma `Paint`: cache labels and ring opts (`EmptyRingOpts` / panel `_ringOpts`). No inline `{...}` args to `Paint*Ring`. Throttle with `Obsidian.Optim.Throttle` when full 60 FPS paint is unnecessary. Hot-path alloc bans: skill **lua** + `lua-hotpath-gate.sh`.
