...
2. Pick the wake mechanism using the host's loop primitive (`/loop` on Cursor, a built-in, not a pstack skill; a scheduled routine or a follow-up session on Devin). An event to watch (CI, a merge, a ref advancing) gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
