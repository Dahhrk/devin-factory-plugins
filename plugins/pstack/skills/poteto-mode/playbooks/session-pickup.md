...
1. Locate the prior trail. A local transcript where the host exposes one (Cursor workspaces keep `agent-transcripts/` under the path the system prompt names — never glob across `~/.cursor/projects/*/`, that crosses workspace boundaries and reads private chats from unrelated projects; a Devin session has no local transcript path), a cloud-session URL, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
