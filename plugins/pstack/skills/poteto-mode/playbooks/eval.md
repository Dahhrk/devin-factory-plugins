...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's transcript where the host exposes one — Cursor workspaces keep `agent-transcripts/` under the path the system prompt names (never glob across `~/.cursor/projects/*/`; that crosses workspace boundaries and reads private chats from unrelated projects). A Devin session has no local transcript path; fall back to each candidate's visible tool-call history. Look at which files each candidate actually opened. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
