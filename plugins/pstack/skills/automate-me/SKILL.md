...
This skill orchestrates three others: an inline mining pass (see step 1), the platform's skill scaffold (`create-skill` on Cursor; a `SKILL.md` written directly on Devin), and the **unslop** skill (prose discipline). It sequences them. It doesn't replace them.
...
Locate the session transcripts before fanning out. Where they live is host-specific: Cursor workspaces keep an `agent-transcripts/` directory named in the system prompt — use only that path, never glob across `~/.cursor/projects/*/` (that crosses workspace boundaries and reads private chats from unrelated projects). A Devin session exposes no local transcript path; ask the operator for a chat export, or skip the mining and interview the user directly.
...
Use the platform's skill scaffold to author the skill (`create-skill` on Cursor; a `SKILL.md` written directly under `.devin/skills/` on Devin). Placement:
