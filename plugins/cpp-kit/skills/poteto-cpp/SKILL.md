...
1. `bash scripts/cpp-rg-gate.sh <product-root> [paths...]` exits 0 (product copy of pack script; single-walk; requires rg).
2. `bash scripts/cpp-hotpath-gate.sh <product-root> [paths...]` exits 0 (rg-gate wall ≤ `CPP_RG_BUDGET_MS`, default 250ms).
3. `bash scripts/cpp-fmt-gate.sh <product-root>` exits 0.
4. `bash scripts/cpp-warn-gate.sh <product-root>` exits 0.
...
- `clang-format` / `-Wall -Wextra` / clang-tidy wiring stay on
- Public ABI unchanged unless the goal is an ABI break
- Soft Dark Glass design tokens when UI-adjacent; no cyan invent

## Ranked bar

1. Trust boundary (ownership / cast / parse / net) before micro-opts
2. Delete dead path before adding
3. `unique_ptr` / `make_unique` before raw new
4. Named C++ cast before C-style cast
5. Measure (perf counters / sanitizers / tidy) before further micro-opt

Load skill **cpp** as needed. Second smell -> lint/CI/skill (encode-lessons), not more prose.

## Delivery labels (standing)

PR titles, branch names when you can choose them, chat-facing labels, and scorecard headers use **plain work descriptions** only (foundation / increment style). Examples: `Replace raw new with unique_ptr`, `Wire clang-tidy in CI`.
