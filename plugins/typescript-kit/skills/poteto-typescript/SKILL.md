...
4. `bash scripts/ts-oxlint-gate.sh <product-root>` exits 0 (`.oxlintrc.json` encodes factory rules including typeAware floating/misused promises; live oxlint+tsgolint pinned / offline-capable unless `TS_OXLINT_CONFIG_ONLY=1`).
5. Product `npm run typecheck` and `npm run lint` exit 0 when those scripts exist. Product oxlint matches `templates/oxlintrc.json` bar (`no-explicit-any`, `no-non-null-assertion`, `switch-exhaustiveness-check`, typeAware `no-floating-promises`, `no-misused-promises`).
